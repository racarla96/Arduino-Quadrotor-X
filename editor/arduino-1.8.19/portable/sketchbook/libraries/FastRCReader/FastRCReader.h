...
class RCChannelMapper : public FastRCReader
{
  public:
    ...
    void setMap(uint16_t minChannelState, uint16_t midChannelState, uint16_t maxChannelState, uint8_t Channel, float toMin = -1, float toMid = 0, float toMax = 1);
  ...
  protected:
    uint16_t midOffset = 16;
    unsigned long timeout_ = 20000; // in us

    struct channel {
      uint16_t min[PORTCOUNT], mid[PORTCOUNT], max[PORTCOUNT];
      float toMin[PORTCOUNT], toMid[PORTCOUNT], toMax[PORTCOUNT];
    } channel;
};
...
void FastRCReader::addChannel(uint8_t ch) {
  ...
  _channel.frequency[ch] = 1500; // Supposing initial mid value
}
...
void RCChannelMapper::setMap(uint16_t minChannelState, uint16_t midChannelState, uint16_t maxChannelState, uint8_t Channel, float toMin, float toMid, float toMax) {
  //To map the Ports D8 to D13 at 0 to 5
#if PORTS_TO_USE == 1
  ch -= 8;
#endif // PORTS_TO_USE
  ...
  //Is the got Channel a valid Channel?
  if (Channel < 0 || Channel >= PORTCOUNT) return;

  //Channel min and max equal?
  if (minChannelState >= maxChannelState || minChannelState >= midChannelState || midChannelState >= maxChannelState) return;
  if (toMin >= toMax || toMin >= toMid || toMid >= toMax) return;

  channel.min[Channel] = minChannelState;
  channel.mid[Channel] = midChannelState;
  channel.max[Channel] = maxChannelState;
  channel.toMin[Channel] = toMin;
  channel.toMid[Channel] = toMid;
  channel.toMax[Channel] = toMax;
}
...
float RCChannelMapper::getChannel(uint8_t ch) {
  ...
  float res;
  uint16_t mid;
  if(abs(_channel.frequency[ch] - channel.mid[ch]) > midOffset) {
    if(_channel.frequency[ch] > channel.mid[ch]) 
    {
      mid = channel.mid[ch] + midOffset;
      res = (_channel.frequency[ch] - (float)mid) * (channel.toMax[ch] - channel.toMid[ch]) / ((float)channel.max[ch] - (float)mid) + channel.toMid[ch];
      res = constrain(res, channel.toMid[ch], channel.toMax[ch]);
    } else {
      mid = channel.mid[ch] - midOffset;
      res = (_channel.frequency[ch] - (float)channel.min[ch]) * (channel.toMid[ch] - channel.toMin[ch]) / ((float)mid - (float)channel.min[ch]) + channel.toMin[ch];
      res = constrain(res, channel.toMin[ch], channel.toMid[ch]);
    }
  } else {
    res = channel.toMid[ch];
  }
